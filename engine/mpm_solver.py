...
class MPMSolver:
    ...
    def __init__(
        ...
        self.mu_0_list = ti.field(dtype=ti.f32)
        self.lambda_0_list = ti.field(dtype=ti.f32)
        ...
        self.particle.place(self.mu_0_list)
        self.particle.place(self.lambda_0_list)
        if self.quant:
            if not self.use_g2p2g:
    ...
    @ti.func
    def seed_particle_enu(self, i, x, material, color, velocity, emmiter_id, E, nu):
        self.x[i] = x
        self.v[i] = velocity
        self.F[i] = ti.Matrix.identity(ti.f32, self.dim)
        self.color[i] = color
        self.material[i] = material
        self.mu_0_list[i] = E / (2 * (1 + nu))
        self.lambda_0_list[i] = E * nu / ((1 + nu) * (1 - 2 * nu))
    ...
    @ti.kernel
    def seed_enu(self, new_particles: ti.i32, new_material: ti.i32, color: ti.i32,
                 E: ti.f32, nu: ti.f32):
        for i in range(self.n_particles[None],
                       self.n_particles[None] + new_particles):
            self.material[i] = new_material
            ...
            self.seed_particle_enu(i, x, new_material, color,
                               self.source_velocity[None], None, E, nu)
    ...
    def add_cube(self,
        ...
        if E is not None and nu is not None:
            self.seed_enu(num_new_particles, material, color, E, nu)
        else:
    ...
    def add_spikes(
        ...
        if E is not None and nu is not None:
            self.seed_spike_enu(num_particles, sides, radius, width, material, color, E, nu)
        else:
    ...
    @ti.kernel
    def seed_spike_enu(self, new_particles: ti.i32, sides: ti.i32, radius:ti.f32, width:ti.f32,
                     new_material: ti.i32, color: ti.i32, E: ti.f32, nu: ti.f32):
        # seed at center
        self.seed_particle_enu(
            self.n_particles[None], self.fan_center[None], new_material, 0xED553B, 
            self.source_velocity[None], None, E, nu)
        for i in range(self.n_particles[None]+1,
                       self.n_particles[None] + new_particles):
            x = self.random_point_in_unit_spike(sides, radius, width)
            x = self.source_bound[0] + x * self.source_bound[1]
            if ti.math.distance(x, self.fan_center[None]) < self.rod_radius: # highlight the rigeion of omega application
                self.seed_particle_enu(i, x, new_material, 0xED553B,
                               self.source_velocity[None], None, E, nu)
            else:
                self.seed_particle_enu(i, x, new_material, color,
                               self.source_velocity[None], None, E, nu)
