...
def compute_loss():
    for i in range(steps - 1):
        if obs == "full":
            ...
            loss[None] += 0.5 * (dist[0, 0])*1e16 * n_particles# + dist[1, 1]) * 1e16
...
assign_ext_load()
obs='full'
sweep = np.linspace(3000, 5000, 2001*10)
sweep_store = np.array([])
...
for e in tqdm(sweep):
    ...
    assign_E()
    compute_loss()
    sweep_store = np.append(sweep_store, loss[None])
np.save("sweep.npy", sweep_store)
