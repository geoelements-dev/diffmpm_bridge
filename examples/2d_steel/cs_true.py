...
for s in range(steps):
    # print(s)
    substep(s)
    print(np.max(np.abs(x.to_numpy()[s,:,:] - x.to_numpy()[0,:,:])))
...
np.save(f'x_cs_{case}.npy', x.to_numpy())
np.save(f's_cs1_{case}.npy', strain.to_numpy())
np.save(f's_cs_{case}.npy', strain2.to_numpy())
