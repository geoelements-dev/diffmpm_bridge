...
ti.root.dense(ti.i, n_particles).place(C, F, E)
ti.root.dense(ti.ij, n_grid).place(grid_v_in, grid_m_in, grid_a)
ti.root.dense(ti.i, max_steps).dense(ti.jk, n_grid).place(grid_v_out, f_ext)
    ...
    for p in range(n_particles):
        ...
        for i in ti.static(range(3)):
            for j in ti.static(range(3)):
                ...
                weight = w[i][0] * w[j][1]
                ti.atomic_add(grid_v_in[base + offset], weight * (p_mass * v[f, p] +
                                                         affine @ dpos))
                ti.atomic_add(grid_m_in[base + offset], weight * p_mass)

@ti.kernel
def grid_op(f: ti.i32):
    for i, j in ti.ndrange(n_grid, n_grid):     
        inv_m = 1 / (grid_m_in[i, j] + 1e-10) 
        v_out = inv_m * grid_v_in[i, j] + dt * f_ext[f, i, j]
        if i == 3 and j <= 5:
            v_out[0] = 0
            # v_out[1] = 0
        if i ==23 and j <= 5:
        ...
        if f > 0:
            grid_a[i, j] = (1/dt) * (grid_v_out[f, i, j] - grid_v_out[f - 1, i, j])
...
np.save(f'a_{loc}_{file_index}3.npy', a.to_numpy())
