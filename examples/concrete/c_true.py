...
# p_mass = 0.087 * p_vol / 1000 # 0.15 klb/ft3 2400 kg/m3
p_vol = 1
p_mass = 0.087 / 1000 # 0.15 klb/ft3 2400 kg/m3
nu = 0.2
...
node_x_locs = ti.Vector(np.arange(0, 17 / n_grid, 1 / n_grid)* size)
...
def assign_ext_load():
    for t, node in ti.ndrange(max_steps, (5, 21)):
            f_ext[t, node, 5] = [0, -f_ext_scale* e[t, node - 5]]
...
for i in range(Nx):
    for j in range(Ny):
        x[0, j * Nx + i] = [
            ((i)/(Nx) * 60 + 20 + 60/Nx * 0.5)*12, 
            ((j)/(Ny) * 4 + 16 + 4/Ny * 0.5)*12
            ]
...
#     gui.arrow(orig=load_locs_np[s], direction = [0, -0.05], color=0xFF0000, radius=3)
...
np.save('s_c1.npy', strain.to_numpy())
np.save('s_c.npy', strain2.to_numpy())