...
# ti.root.dense(ti.k, max_steps).dense(ti.l, n_particles).place(x, v, C, F, strain2, strain)
# ti.root.dense(ti.i, n_particles).place(E)
# # ti.root.dense(ti.ij, n_grid).place()
# ti.root.dense(ti.i, max_steps).dense(ti.jk, n_grid).place(grid_v_out, f_ext, grid_v_in, grid_m_in)
...
def p2g(f: ti.i32):
    for p in range(n_particles):
        ...
        strain[f + 1, p] += 0.5 * (new_F.transpose() @ new_F - ti.math.eye(dim))
        for i in ti.static(range(3)):
            for j in ti.static(range(3)):
                offset = ti.Vector([i, j])
                dpos = (ti.cast(ti.Vector([i, j]), real) - fx) * dx
                weight = w[i][0] * w[j][1]
                ti.atomic_add(grid_v_in[f, base + offset], weight * (p_mass * v[f, p] +
                                                         affine @ dpos))
                ti.atomic_add(grid_m_in[f, base + offset], weight * p_mass)
...
def assign_E():
    for i in range(Nx):
        for j in range(Ny):
            x[0, j * Nx + i] = [
                ((i)/(Nx) * 0.5 + 0.25 + 0.5 / Nx * 0.5 + (1/8 * 0.25)) * size, 
...
np.save(f's1.npy', strain.to_numpy())
np.save(f's.npy', strain2.to_numpy())
