...
def assign_E_modular(
    ...
    n_targets: int, 
    target_indices: ti.types.ndarray, 
...
def compute_loss_and_grad_modular(
        params,
        background_value,
        n_targets,
        target_indices,
    ...
    for i in range(n_blocks):
        E_block[i] = params[i]
    with ti.ad.Tape(loss=loss):
        assign_E_modular(background_value, n_targets, target_indices, target_values)
        assign_ext_load()
        for s in range(steps - 1):
            substep(s)
        compute_loss()
...
target_indices = exceed
target_values = result_naive[exceed]


result_2 = minimize(compute_loss_and_grad_modular,
                    np.array(initial_params),
                    args=(background_value, n_targets, target_indices, target_values),
                    method='L-BFGS-B',
                    jac=True,
                    callback=callback_fn,
                    options=options)
...
# filename = f"result_{case}_{n_blocks_x}_{n_blocks_y}_{obs}"
