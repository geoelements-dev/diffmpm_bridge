    ...
    for i in range(n_sub_blocks):
        for j in range(sub_block_x):
            for k in range(sub_block_y):
                E[sub_block_index[i] + j + k * Nx] = sub_block_values[i]
...
for z in range(3):
    ...
    # subdivide block
    n_sub_blocks = len(exceed)
    sub_block_x = sub_block_order[z]
    sub_block_y = sub_block_order[z]
    sub_block_index = ti.field(dtype=int, shape=(n_sub_blocks))
    sub_block_values = ti.field(dtype=real, shape=(n_sub_blocks), needs_grad=True)
...
filename = f"result_c2f/result_{case}_mask_{n_blocks_x}_{n_blocks_y}_{obs}_start_{mask_start}"
