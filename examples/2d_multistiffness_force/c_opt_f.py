...
velocity = ti.field(dtype=real, shape=(), needs_grad=True)
width = ti.field(dtype=real, shape=(), needs_grad=True)
...
def compute_loss():
    for i in range(steps - 1):
        if obs == "full":
            ...
            for j in range(n_particles*0.5):
                dist = (target_strain[i, j] - strain2[i, j]) ** 2
                loss[None] += 0.5 * (dist[0, 0])*1e16 * 2# + dist[1, 1]) * 1e16
...
# e_np = np.exp(-a * t * t)
e = ti.field(ti.f32, (steps, n_grid), needs_grad=True)
a = ti.field(dtype=real, shape=(), needs_grad=True)
# e.from_numpy(e_np)

bw = 0.5
bwr = -6.0
ref = ti.pow(10.0, bwr / 20.0)

import math

@ti.kernel
def assign_ext_load():
    for i, j in ti.ndrange(max_steps, n_grid):  # i: time step, j: node
        node_x_loc = j / n_grid
        time_to_center = node_x_loc / (velocity[None] * 12)
        t = i * dt - time_to_center
        e[i, j] = ti.exp((math.pi * width[None] * 0.5) ** 2 / (4.0 * ti.log(ti.pow(10.0, -6 / 20.0))) * t * t)

    for t, node in ti.ndrange(max_steps, n_grid):
            f_ext[t, node, 10] = [0, -force[None] * e[t, node]]
...
if optim == 'grad':
    ...
    for j in range(grad_iterations):
        ...
        params = [E1, E2, E3, force, velocity, width]
        param_vals = [E1[None], E2[None], E3[None], force[None], velocity[None], width[None]]
        for i in range(n_params):
            ...
            param_vals[i] -= lr[i] * m_adam[i] / (ti.sqrt(v_hat[i]) + epsilon)
    ...
    with open("result_full_grad_c.json", "w") as outfile: 
    ...
    n_ef_it = 3
    E1_hist, E2_hist, E3_hist, E4_hist = [], [], [], []
    F_hist, V_hist, W_hist = [], [], []
    it_hist = []
    ...
    def compute_loss_and_grad_f(params):
        ...
        loss_val = loss[None]
        grad_val = [force.grad[None], velocity.grad[None], width.grad[None]]
    ...
    def callback_fn_f(intermediate_result):
        ...
        V_hist.append(params[1])
        W_hist.append(params[2])
        print(j, 
            'loss=', loss, 
            '   grad=', grad,
            '   params=', params)
    ...
    init_e = 4e3
    initial_params = [init_e, init_e, init_e, init_e, 45, 90, 18]
    params = initial_params
    obs_choices = ["full", "half", "row"]
    obs = obs_choices[0]
    ...
    V_hist.append(initial_params[5])
    W_hist.append(initial_params[6])
    ...
    with open(f"result_c_{init_e}_{obs}_f.json", "w") as outfile: 
        json.dump(result_dict, outfile)
