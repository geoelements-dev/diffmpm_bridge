...
#             f_ext[t, node, 10] = [0, -f_ext_scale / ((stop - start + 1))]
...
if optim == 'grad':
    ...
    with open(f"result_c_4000.0_row.json", 'r') as file:
        checkpoint = json.load(file)["checkpoint"]
    ...
    init_e = 4e3
    initial_params = [init_e, init_e, init_e, init_e, 45]
    ...
    for i in range(n_ef_it):
        ...
        if i > checkpoint:
            ...
            with open(f"result_c_{init_e}_{obs}.json", "w") as outfile: 
                json.dump(result_dict, outfile)
