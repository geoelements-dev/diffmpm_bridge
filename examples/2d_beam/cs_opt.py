...
def compute_loss():
    for i in range(steps - 1):
        if obs == "full":
            ...
            #     loss[None] += 0.5 * (dist[0])*1e20# + dist[1, 1]) * 1e16
        elif obs == "row":
            for j in range(Nx):
                dist = (target_strain[i, j] - strain2[i, j]) ** 2
                loss[None] += 0.5 * (dist[0, 0])*1e16*Ny# + dist[1, 1]) * 1e16
                # dist = (target_disp[i, j] - disp[i, j]) ** 2
                # loss[None] += 0.5 * (dist[0])*1e30# + dist[1, 1]) * 1e16
        elif obs == "sensor":
            for j in range(16):
                dist = (target_strain[i, j*12] - strain2[i, j*12]) ** 2
                loss[None] += 0.5 * (dist[0, 0])*1e16*12*Ny# + dist[1, 1]) * 1e16
                # dist = (target_disp[i, j] - disp[i, j]) ** 2
                # loss[None] += 0.5 * (dist[0])*1e1# + dist[1, 1]) * 1e16
...
if optim == 'lbfgs':
    ...
    for i in range(n_ef_it):
        ...
        result = minimize(compute_loss_and_grad_e, 
                        ...
                        jac=True, 
                        hess='2-point',
                        bounds=[(b1,b2),(b1,b2),(b1,b2),(b1,b2)],
